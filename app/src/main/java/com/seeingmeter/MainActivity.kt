...
class MainActivity : Activity(), CameraController.Listener, SensorEventListener, LocationListener {
    ...
    private val historyR0 = ArrayDeque<Pair<Long, Double>>()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Config.load(this)
        analyzer.flatProfile = FlatStore.load(this, Config.cameraId)
        logger = DataLogger(this)
        buildUi(); ensurePermissions()
        sensorMgr = getSystemService(Context.SENSOR_SERVICE) as SensorManager
        sensorMgr?.getDefaultSensor(Sensor.TYPE_PRESSURE)?.let { sensorMgr?.registerListener(this, it, SensorManager.SENSOR_DELAY_NORMAL) }
    ...
    override fun onDestroy() {
        ...
        sensorMgr?.unregisterListener(this); locMgr?.removeUpdates(this)
    }
    ...
    private fun buildUi() {
        ...
        scroll.addView(col)
        preview = TextureView(this).apply { layoutParams = LinearLayout.LayoutParams(MATCH_PARENT, dp(180)) }
        col.addView(preview)
        tvInfo = mkTv(); col.addView(tvInfo)
        tvSignal = mkTv(); col.addView(tvSignal)
        tvMeta = mkTv(); col.addView(tvMeta)
        tvResults = mkTv().apply { setTextSize(TypedValue.COMPLEX_UNIT_SP, 13f) }; col.addView(tvResults)
        tvParams = mkTv(true); col.addView(tvParams)
        tsView = TimeSeriesView(this).apply { layoutParams = LinearLayout.LayoutParams(MATCH_PARENT, dp(110)) }; col.addView(tsView)
        specView = SpectrumView(this).apply { layoutParams = LinearLayout.LayoutParams(MATCH_PARENT, dp(140)) }; col.addView(specView)
        ...
        for (b in arrayOf(btnMeasure, btnSave, btnReset, btnFlat, btnSettings)) {
            b.layoutParams = LinearLayout.LayoutParams(0, WRAP_CONTENT, 1f); btnRow.addView(b) }
        col.addView(btnRow); setContentView(scroll)
        btnMeasure.setOnClickListener { if (measuring) stopMeasuring() else startMeasuring() }
        btnSave.setOnClickListener { val path = logger.finalize(); Toast.makeText(this, "saved: $path", Toast.LENGTH_LONG).show() }
    ...
    private fun startMeasuring() {
        ...
        logger = DataLogger(this).also {
            ...
            val path = it.open(); tvParams.text = "JSON: $path"
        }
        ...
        measuring = true; btnMeasure.text = "⏸ Stop"; startLocation(); maybeFetchMeteoblue()
    }

    private fun stopMeasuring() { if (!measuring) return; camera.stop(); measuring = false; btnMeasure.text = "▶ Measure" }

    private fun resetSession() {
        stopMeasuring(); analyzer.reset(); historyR0.clear(); lastResult = null
        tsView.invalidate(); specView.invalidate()
    }

    private fun captureFlat() {
        if (!measuring) { Toast.makeText(this, "Start measuring first, then tap Flat", Toast.LENGTH_LONG).show(); return }
    ...
    private fun refreshTexts() {
        ...
        val fs = f.fullScale
        val actMs = f.actualExposureNs / 1e6; val setMs = f.setExposureNs / 1e6
        val expStr = if (f.actualExposureNs > 0) "Exp=${"%.2f".format(actMs)}ms(set ${"%.2f".format(setMs)})"
                     else "Exp(set)=${"%.2f".format(setMs)}ms"
        val isoStr = if (f.actualIso > 0) "ISO=${f.actualIso}(set ${f.setIso})" else "ISO(set)=${f.setIso}"
        val aeStr = if (f.aeOff) "AE:OFF" else "AE:ON⚠"
        tvInfo.text = "SeeingMeter-${Config.APP_VERSION}  $expStr  $isoStr  $aeStr  ~${"%.0f".format(f.sampleRateHz/1000.0)}kHz  cam=${Config.cameraId}  rows=${f.totalRows}"
        val flag = when {
            f.peakCounts >= fs - 1 -> "  🔴 OVERFLOW"; f.satRows > 0 -> "  🟠 ${f.satRows} sat"
            f.meanCounts < fs*0.05 -> "  🔵 underflow"; else -> "  ✓" }
        tvSignal.text = "peak=${f.peakCounts.toInt()}/${fs.toInt()} (${"%.0f".format(f.peakCounts/fs*100)}%)" +
                "   mean=${f.meanCounts.toInt()}/${fs.toInt()} (${"%.0f".format(f.meanCounts/fs*100)}%)$flag"
        val gpsStr = if (lat != null) "GPS ${"%.4f".format(lat!!)},${"%.4f".format(lon!!)} ±${accM?.toInt()?:0}m" else "GPS —"
        tvMeta.text = "$gpsStr${pressureHpa?.let{"  ·  ${"%.1f".format(it)} hPa"}?:""}"
        val r = lastResult
        if (r != null) tvResults.text = "r0=${"%.2f".format(r.r0M*100)}cm  seeing=${"%.2f".format(r.seeingArcsec)}\"" +
                "  σ_I=${"%.4f".format(r.sigma2I)}  f_c=${"%.1f".format(r.fCharHz)}Hz  H=${"%.1f".format(r.hFitM/1000)}km  ${r.quality}"
    ...
    override fun onError(msg: String) { runOnUiThread { Toast.makeText(this, msg, Toast.LENGTH_LONG).show() } }
    override fun onSensorChanged(e: SensorEvent) {
        if (e.sensor.type == Sensor.TYPE_PRESSURE) { pressureHpa = e.values[0].toDouble(); logger.setPressureHpa(pressureHpa) }
    }
    override fun onAccuracyChanged(s: Sensor?, a: Int) {}
    override fun onLocationChanged(loc: Location) {
        lat=loc.latitude; lon=loc.longitude; accM=loc.accuracy
        altM=if(loc.hasAltitude())loc.altitude else null; logger.setGps(lat,lon,accM,altM)
    }
    ...
    private fun startLocation() {
        ...
        try { locMgr?.requestLocationUpdates(LocationManager.GPS_PROVIDER, 10_000L, 0f, this) } catch (_: SecurityException) {}
    }
    ...
    private fun showSettingsDialog() {
        ...
        fun row(label: String, view: View) { box.addView(TextView(this).apply { text = label; setPadding(0, dp(8), 0, 0) }); box.addView(view) }
        val camSpin = Spinner(this).apply {
            adapter = ArrayAdapter(this@MainActivity, android.R.layout.simple_spinner_item, labels).also {
                it.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item) }; setSelection(curIdx) }
        row("Camera", camSpin)
        fun numEdit(d: String): EditText = EditText(this).apply { inputType = InputType.TYPE_CLASS_NUMBER or InputType.TYPE_NUMBER_FLAG_DECIMAL; setText(d) }
        ...
        row("Exposure target (0..1)", eTarget); row("Exposure tolerance (±)", eTol)
        row("Band low (Hz)", eBandLo); row("Band high (Hz)", eBandHi)
        row("Wind (m/s)", eWind); row("Layer height (m)", eH)
        row("COEFF (calibration)", eCoeff); row("Meteoblue API key", eKey)
        AlertDialog.Builder(this).setTitle("Settings — SeeingMeter-${Config.APP_VERSION}")
            ...
            .setPositiveButton("OK") { _, _ ->
                ...
                Config.save(this)
                analyzer.flatProfile = FlatStore.load(this, Config.cameraId)
                if (measuring) { stopMeasuring(); startMeasuring() }
            }.setNegativeButton("Cancel", null).show()
    }

    class TimeSeriesView(c: Context) : View(c) {
        var points: List<Pair<Long, Double>> = emptyList()
        private val pLine = Paint().apply { color = Color.YELLOW; strokeWidth = 3f; isAntiAlias = true }
        private val pAxis = Paint().apply { color = Color.DKGRAY; strokeWidth = 1f }
        private val pTxt  = Paint().apply { color = Color.LTGRAY; textSize = 24f }
        override fun onDraw(canvas: Canvas) {
            canvas.drawColor(Color.BLACK); val w=width.toFloat(); val h=height.toFloat()
            canvas.drawLine(0f,h-1,w,h-1,pAxis); canvas.drawText("r0 history (2 min) cm",8f,24f,pTxt)
            if(points.isEmpty())return
            val t0=points.first().first; val t1=points.last().first; val span=max(1L,t1-t0).toFloat()
            val ymax=(points.maxOf{it.second}*100).coerceAtLeast(20.0).toFloat()
            var px=-1f; var py=0f
            for((t,r0) in points){val x=((t-t0)/span)*w; val y=h-((r0*100).toFloat()/ymax)*(h-30f)
                if(px>=0)canvas.drawLine(px,py,x,y,pLine); px=x; py=y}
        }
    }

    class SpectrumView(c: Context) : View(c) {
        private var freqs=DoubleArray(0); private var psd=DoubleArray(0)
        ...
        fun set(f:DoubleArray,p:DoubleArray,fc:Double,fF:Double){freqs=f;psd=p;fChar=fc;fFresnel=fF;invalidate()}
        override fun onDraw(canvas:Canvas){
            canvas.drawColor(Color.BLACK); val w=width.toFloat(); val h=height.toFloat()
            canvas.drawText("PSD  magenta=f_char  green=f_F",8f,22f,pTxt)
            if(freqs.size<2||psd.size<2)return
            val fLo=max(1.0,freqs[1]);val fHi=freqs.last()
            val pLo=(psd.filter{it>0}.minOrNull()?:1e-12).coerceAtLeast(1e-15);val pHi=psd.maxOrNull()?:1.0
            fun mapX(f:Double)=((log10(f)-log10(fLo))/(log10(fHi)-log10(fLo))*w).toFloat()
            fun mapY(p:Double):Float{val pp=p.coerceAtLeast(pLo);return(h-((log10(pp)-log10(pLo))/(log10(pHi)-log10(pLo)))*(h-30f)).toFloat()}
            var px=-1f;var py=0f
            for(i in 1 until freqs.size){if(freqs[i]<fLo)continue;val x=mapX(freqs[i]);val y=mapY(psd[i])
                if(px>=0)canvas.drawLine(px,py,x,y,pPsd);px=x;py=y}
